...
main = hspec $ do
  describe "Utilizando Diccionarios" $ do
    it "puede determinarse si un elemento es una clave o no" $ do
      ...
      belongs "Argentina" [("Pedro", ["Berlin", "Paris"]), ("Pablo", ["Argentina"]), ("Juan", ["Peru", "Amsterdam"])] `shouldBe` False
      [("H", [1]), ("E", [2]), ("Y", [0])] ? "R"  `shouldBe` False
      [("V", [1]), ("O", [2]), ("S", [0])] ? "V"  `shouldBe` True
      [] ? "V"  `shouldBe` False
      [("Pedro", ["Berlin", "Paris"]), ("Pablo", ["Argentina"]), ("Juan", ["Peru", "Amsterdam"])] ? "Juana" `shouldBe` False
    ...
    it "debe devolver el valor asociado a la clave dada, asumiendo que está definida" $ do  
      ...
      [("Corrientes", [45, 30, 55]), ("República", [67, 43, 110]), ("Pacífico", [80, 33, 21, 11]), ("Alvear", [])] ! "Alvear" `shouldBe` []
    ...
    it "agregar el nuevo valor con la función dada" $ do
      ...
      insertWith (\n m -> if (n - m) < 0 then 0 else (n-m)) " pan " 80 [(" café ", 30),(" galletita ", 20), (" pan ", 15), (" chocolate ", 3)] `shouldBe` [(" café ", 30),(" galletita ", 20), (" pan ", 0), (" chocolate ", 3)]
      insertWith (\n m -> if (n - m) < 0 then 0 else (n-m)) " agua " 10 [(" café ", 30),(" galletita ", 20), (" pan ", 15), (" chocolate ", 3)] `shouldBe` [(" café ", 30),(" galletita ", 20), (" pan ", 15), (" chocolate ", 3), (" agua ", 10)]
    ...
    it "agrupa por clave en algún orden" $ do
      ...
      groupByKey [("fruta", "Banana"), ("carne", "Picada"), ("verdura", "Espinaca")] `shouldMatchList` [("fruta", ["Banana"]), ("carne", ["Picada"]), ("verdura", ["Espinaca"])]


    it "divide la carga de manera balanceada entre una cantidad determinada de máquinas en algún orden" $ do
      distributionProcess 5 ["Alvear", "Corrientes", "Rosario", "Chaco", "Independencia", "Pacífico Rodriguez", "Pueyrredon", "Witcomb"] `shouldMatchList` [["Alvear", "Pacífico Rodriguez"], ["Corrientes", "Pueyrredon"], ["Rosario", "Witcomb"], ["Chaco"], ["Independencia"]]
    ...
    it "aplica el mapper a cada uno de los elementos y luego agrupa los resultados por clave" $ do
      mapperProcess (\x -> [(x,1)]) ["Pablo", "Pedro", "Pepe", "Guillermo", "Aldana"] `shouldMatchList` [("Pablo",[1]), ("Pedro",[1]), ("Pepe",[1]), ("Guillermo",[1]), ("Aldana",[1])]
      mapperProcess (\x -> [(x,1)]) ["Pablo", "Pedro", "Pepe", "Pedro", "Pedro", "Guillermo", "Aldana"] `shouldMatchList` [("Pablo",[1]), ("Pedro",[1,1,1]), ("Pepe",[1]), ("Guillermo",[1]), ("Aldana",[1])]
      mapperProcess (\(n,p) -> [(n,p),(n,p+p*5)]) [("Papa",32), ("Banana", 39), ("Manzana", 19), ("Tomate", 24), ("Banana", 58)] `shouldMatchList` [("Papa",[192,32]), ("Banana", [348,58,234,39]), ("Manzana", [114,19]), ("Tomate", [144,24])]


    it "combina los resultados de cada máquina y los combina agrupandolos por clave (ordenandolos en forma creciente)" $ do
      combinerProcess [[("Pablo",[1]), ("Pedro",[1,1,1])], [("Pepe",[1])], [("Guillermo",[1]), ("Aldana",[1])]] `shouldBe` [("Aldana",[1]), ("Guillermo",[1]), ("Pablo",[1]), ("Pedro",[1,1,1]), ("Pepe",[1])]
      combinerProcess [[(56,["Amsterdam", "Florencia"]), (23, ["Berlin"])], [(56,["Ulm", "Buenos Aires"]),(87,["Viena","Paris","Amsterdam"]),(23,["Berlin"])], [(77,["Chile", "Nueva York", "Tokio"])], [(1,["Paraguay"])]] `shouldBe` [(1,["Paraguay"]), (23, ["Berlin", "Berlin"]), (56,["Amsterdam", "Florencia", "Ulm", "Buenos Aires"]), (77,["Chile", "Nueva York", "Tokio"]), (87,["Viena","Paris","Amsterdam"])]
    ...
    it "aplica el reducer sobre cada par de elementos y luego aplana el resultado para unificar las soluciones" $ do
      ...
      reducerProcess (\(n,l) -> [l]) [("Chocotorta", ["Chocolinas", "Dulce de Leche", "Crema"]), ("Galletitas de Limón", ["Limón", "Harina", "Azúcar"]), ("Budín de Banana", ["Banana", "Harina", "Manteca"])] `shouldBe` [["Chocolinas", "Dulce de Leche", "Crema"], ["Limón", "Harina", "Azúcar"], ["Banana", "Harina", "Manteca"]]


    it "aplica la técnica MapReduce para 100 máquinas" $ do
      mapReduce (\(nom,ciudad) -> [(ciudad,1)]) (\(ciudad,cant) -> [(ciudad, sum cant)]) [("Pablo","Berlin"), ("Gabriela","Amsterdam"), ("Taihú","Cairo"), ("Pablo", "Cairo"), ("Taihú","Amsterdam"), ("Juan","Amsterdam")] `shouldBe` [("Amsterdam", 3), ("Berlin", 1), ("Cairo", 2)]
